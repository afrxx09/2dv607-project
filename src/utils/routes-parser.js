...
function createLink(node, parentPath){
    ...
    if (!node.component){//no component
        if(!children.length){//no children
            return null;//don't render
        }
        
        if(hasIndexRoute){
            children.splice(children.indexOf(indexRoute), 1); //remove indexroute from children
            if(!node.title){//node has no title attr
                title = getTitle(indexRoute);//take title from index route instead
            }
        }
        if(children.length){//if there are children after index is removed
